...
func (m *messageThenFinalProvider) GetDefaultModel() string {
	return "mock-message-then-final"
}

func TestAgentLoop_Run_PublishesFinalWhenDifferentFromMessageTool(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "agent-test-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	...
	defer waitCancel()
	first, ok := msgBus.SubscribeOutbound(waitCtx)
	if !ok {
		t.Fatal("expected first outbound message")
	}
	second, ok := msgBus.SubscribeOutbound(waitCtx)
	if !ok {
		t.Fatal("expected second outbound message")
	}

	if first.Content != "正在处理，请稍等" {
		t.Fatalf("unexpected first message: %q", first.Content)
	}
	if second.Content != "已完成：生成成功" {
		t.Fatalf("unexpected second message: %q", second.Content)
	}
