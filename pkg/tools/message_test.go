...
func TestMessageTool_HasSentMatchingInRound(t *testing.T) {
	tool := NewMessageTool()
	tool.SetContext("test-channel", "test-chat-id")
	tool.SetSendCallback(func(channel, chatID, content string) error {
		return nil
	...
	ctx := context.Background()
	result := tool.Execute(ctx, map[string]any{"content": "  done  "})
	if result.IsError {
		t.Fatalf("unexpected error: %s", result.ForLLM)
	}

	if !tool.HasSentMatchingInRound("test-channel", "test-chat-id", "done") {
		t.Fatal("expected matching message to be found")
	}
	if tool.HasSentMatchingInRound("test-channel", "test-chat-id", "different") {
		t.Fatal("did not expect non-matching message to be found")
	}
	if tool.HasSentMatchingInRound("other-channel", "test-chat-id", "done") {
		t.Fatal("did not expect match across different channels")
	}
}

func TestMessageTool_SetContextResetsRoundHistory(t *testing.T) {
	tool := NewMessageTool()
	tool.SetContext("test-channel", "test-chat-id")
	tool.SetSendCallback(func(channel, chatID, content string) error {
		return nil
	...
	ctx := context.Background()
	_ = tool.Execute(ctx, map[string]any{"content": "done"})
	if !tool.HasSentMatchingInRound("test-channel", "test-chat-id", "done") {
		t.Fatal("expected match before context reset")
	}

	tool.SetContext("test-channel", "test-chat-id")
	if tool.HasSentMatchingInRound("test-channel", "test-chat-id", "done") {
		t.Fatal("expected round history to reset after SetContext")
	}
}
