...
func authHelp() {
	...
	fmt.Println("  kingclaw auth login --provider openai")
	fmt.Println("  kingclaw auth login --provider openai --device-code")
	fmt.Println("  kingclaw auth login --provider anthropic")
	fmt.Println("  kingclaw auth login --provider google-antigravity")
	fmt.Println("  kingclaw auth models")
	fmt.Println("  kingclaw auth logout --provider openai")
	fmt.Println("  kingclaw auth status")
}
...
func authLoginGoogleAntigravity() {
	...
	fmt.Println("Try it: kingclaw agent -m \"Hello world\"")
}
...
func authStatusCmd() {
	...
	if len(store.Credentials) == 0 {
		...
		fmt.Println("Run: kingclaw auth login --provider <name>")
		return
	}
...
func authModelsCmd() {
	...
	if err != nil || cred == nil {
		...
		fmt.Println("Run: kingclaw auth login --provider google-antigravity")
		return
	}
