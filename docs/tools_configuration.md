...
KingClaw's tools configuration is located in the `tools` field of `config.json`.
...
By default, KingClaw blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `KINGCLAW_TOOLS_<SECTION>_<KEY>`:

For example:
- `KINGCLAW_TOOLS_WEB_BRAVE_ENABLED=true`
- `KINGCLAW_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `KINGCLAW_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
