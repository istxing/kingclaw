# Using Antigravity Provider in KingClaw

This guide explains how to set up and use the **Antigravity** (Google Cloud Code Assist) provider in KingClaw.

> Scope: end-user setup, authentication, and troubleshooting.
> For protocol/API implementation details, see `docs/ANTIGRAVITY_AUTH.md`.
...
```bash
kingclaw auth login --provider antigravity
```
...
6.  **Paste it back into the terminal** where KingClaw is waiting.

KingClaw will extract the authorization code and complete the process automatically.
...
### Switch Models
You can change the default model in `~/.kingclaw/config.json` or override it via the CLI:

```bash
# Override for a single command
kingclaw agent -m "Hello" --model claude-opus-4-6-thinking
```
...
1.  **Environment Variables**:
    *   `KINGCLAW_AGENTS_DEFAULTS_MODEL=gemini-flash`
2.  **Authentication persistence**: 
    If you've logged in locally, you can copy your credentials to the server:
    ```bash
    scp ~/.kingclaw/auth.json user@your-server:~/.kingclaw/
    ```
...
*   **429 Rate Limit**: Antigravity has strict quotas. KingClaw will display the "reset time" in the error message if you hit a limit.
*   **404 Not Found**: Ensure you are using a model ID from the `kingclaw auth models` list. Use the short ID (e.g., `gemini-3-flash`) not the full path.
